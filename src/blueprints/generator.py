...
class CodeGenerator:
    ...
    def generate_project(self, resolved: ResolvedBlueprint, output_dir: Path, language: str = "python", force: bool = False, main_md_path: Optional[Path] = None, verify: bool = True) -> Dict[str, Path]:
        ...
        for blueprint in resolved.generation_order:
            try:
                code, output_path = self._generate_single_blueprint_file(
                    blueprint, resolved, dependency_versions, language, 
                    output_dir, force, verify, main_md_path, generated_context
                )
        ...
        # Generate Makefile
        makefile_path = self._generate_project_makefile(resolved, main_md_path, language, force)
        if makefile_path:
            generated_files["Makefile"] = makefile_path
        
        return generated_files
    
    def _extract_dependency_versions_safe(self, main_md_path: Optional[Path]) -> Dict[str, str]:
    ...
    def _generate_single_blueprint_file(self, blueprint: 'Blueprint', resolved: ResolvedBlueprint, 
                                      dependency_versions: Dict[str, str], language: str, 
                                      output_dir: Path, force: bool, verify: bool,
                                      main_md_path: Optional[Path], generated_context: Dict[str, str]) -> tuple[str, Path]:
        """Generate code for a single blueprint and return code and output path."""
        context_parts = self._create_blueprint_context(blueprint, resolved, generated_context, language)
    ...
    def _create_blueprint_context(self, blueprint: 'Blueprint', resolved: ResolvedBlueprint, 
    ...
    def _generate_code_with_verification(self, blueprint: 'Blueprint', context_parts: List[str], 
                                       language: str, verify: bool, main_md_path: Optional[Path]) -> str:
        ...
        code, verification_results = self.generate_with_verification(
            blueprint, context_parts, language, max_retries=2, 
            project_root=project_root, main_md_path=main_md_path
        )
        
        self._log_verification_warnings(blueprint.module_name, verification_results)
        return code
    
    def _log_verification_warnings(self, module_name: str, verification_results: List['VerificationResult']) -> None:
    ...
    def _generate_project_makefile(self, resolved: ResolvedBlueprint, main_md_path: Optional[Path], 
                                 language: str, force: bool) -> Optional[Path]:
        """Generate project Makefile."""
        project_root = self._find_project_root(resolved, main_md_path)
        return self._generate_makefile(resolved, project_root, language, force, main_md_path)
    
    def generate_single_with_context(self, resolved: ResolvedBlueprint, output_path: Path, language: str = "python", force: bool = False, verify: bool = True) -> Path:
        ...
        prompt = self._create_single_blueprint_prompt(resolved.main, language, context_parts, dependency_versions)
        
        # Generate code with full context and optional verification
        try:
            if verify:
                # Use verification loop for better code quality
                project_root = resolved.main.file_path.parent if resolved.main.file_path else output_path.parent
                # Find main.md by walking up the directory tree
                main_md_for_verification = self._find_main_md_in_project(project_root)
                
                code, verification_results = self.generate_with_verification(
                    resolved.main, context_parts, language, max_retries=2,
                    project_root=project_root, main_md_path=main_md_for_verification
                )
                # Log verification results if needed
                failed_verifications = [r for r in verification_results if not r.success]
                if failed_verifications:
                    print(f"Warning: Blueprint {resolved.main.module_name} has verification issues:")
                    for result in failed_verifications:
                        print(f"  - {result.error_type}: {result.error_message}")
            else:
    ...
    def _create_single_blueprint_prompt(self, blueprint: Blueprint, language: str, context_parts: List[str], dependency_versions: Optional[Dict[str, str]] = None) -> str:
    ...
    def _extract_dependency_versions(self, main_md_path: Path) -> Dict[str, str]:
        ...
        for line in content.splitlines():
            ...
            if "## Third-party Dependencies" in line or "## Dependencies" in line:
            ...
            if in_deps_section and line.startswith("- "):
                # Match patterns like: - fastapi>=0.104.0
                ...
                if match:
                    ...
                    dependency_versions[name] = f"{operator}{version}" if version != "latest" else version
        
        return dependency_versions
    
    def _create_python_init_files(self, generated_files: Dict[str, Path], force: bool = False) -> Dict[str, Path]:
    ...
    def _generate_makefile(self, resolved: ResolvedBlueprint, output_dir: Path, language: str, force: bool, main_md_path: Optional[Path] = None) -> Optional[Path]:
    ...
    def _create_makefile_content(self, project_name: str, dependencies: list, dev_dependencies: list, 
                                install_commands: list, run_commands: list, env_vars: list, language: str, app_module: Optional[str] = None) -> str:
    ...
    def generate_with_verification(self, blueprint: 'Blueprint', context_parts: List[str], language: str = "python", max_retries: int = 3, project_root: Optional[Path] = None, main_md_path: Optional[Path] = None) -> Tuple[str, List['VerificationResult']]:
        ...
        verifier = GenerationVerifier(max_retries=max_retries, language=language, project_root=project_root)
        return verifier.verify_and_improve(self, blueprint, context_parts, language, main_md_path)
    
    def generate_single_blueprint(self, blueprint: 'Blueprint', context_parts: List[str], language: str = "python", dependency_versions: Optional[Dict[str, str]] = None) -> str:
