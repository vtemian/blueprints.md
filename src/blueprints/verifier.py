...
class CodeVerifier:
    ...
    def verify_imports(
        self, code: str, project_root: Path, main_md_path: Optional[Path] = None
    ) -> VerificationResult:
    ...
    def verify_types(self, code: str, file_path: Path) -> VerificationResult:
        ...
        try:
            ...
            return VerificationResult(
                success=True, warnings=["mypy not available for type checking"]
            )
    ...
    def verify_runtime(self, code: str, file_path: Path) -> VerificationResult:
        ...
        try:
            ...
            return VerificationResult(
                success=False, error_type="runtime", error_message=str(e)
            )
    ...
    def verify_all(
        ...
        if blueprint:
            ...
            results.append(self.verify_blueprint_requirements(code, blueprint))
    ...
    def verify_blueprint_dependencies(
        self, code: str, blueprint: "Blueprint"
    ) -> VerificationResult:
        """Verify that code imports match blueprint dependencies."""
        if not blueprint.blueprint_refs:
            return VerificationResult(success=True)

        expected_imports = self._extract_expected_imports_from_refs(
        ...
        for expected in expected_imports:
            if not self._import_matches(expected, actual_imports):
                errors.append(
                    f"Missing or incorrect import: expected '{expected['statement']}' but found mismatched import"
                )
        ...
        for actual in actual_imports:
            if self._has_relative_import(actual):
                errors.append(
                    f"Relative import found: '{actual}' - top-level modules should use absolute imports matching blueprint dependencies"
                )
            ...
            if function in code and function not in existing_imports:
                ...
                if not function_imported:
                    missing_imports.append(
                        f"Function '{function}' is used but not imported. Add: {import_statement}"
                    )
    ...
    def verify_async_await_usage(
        self, code: str, blueprint: Optional["Blueprint"] = None
    ) -> VerificationResult:
    ...
    def _extract_expected_imports_from_refs(
        self,
        ...
        for dep in dependencies:
            ...
            if "[" in dep and "]" in dep:
                ...
                if " as " in import_part:
                    ...
                    statement = (
                        f"from {module_part} import {item.strip()} as {alias.strip()}"
                    )
                else:
                ...
                expected.append(
                    {
                        "module": module_part,
                        "item": (
                            import_part.split(" as ")[0].strip()
                            if " as " in import_part
                            else import_part.strip()
                        ),
                        "alias": (
                            import_part.split(" as ")[1].strip()
                            if " as " in import_part
                            else None
                        ),
                        "statement": statement,
    ...
    def _import_matches(self, expected: dict, actual_imports: List[str]) -> bool:
        ...
        for actual in actual_imports:
            if self._normalize_import(expected_statement) == self._normalize_import(
                actual
            ):
                return True
    ...
    def _is_expected_relative_import(
        self, actual_import: str, expected_imports: List[dict]
    ) -> bool:
    ...
    def _run_mypy_check(self, file_path: Path) -> VerificationResult:
        ...
        try:
            ...
            if exit_code == 0:
                return VerificationResult(success=True)
            return VerificationResult(
                success=False, error_type="type", error_message=stdout or stderr
            )
        except ImportError:
            return VerificationResult(
                success=True, warnings=["mypy not available for type checking"]
    ...
    def verify_blueprint_requirements(self, code: str, blueprint: "Blueprint") -> VerificationResult:
        """Verify that generated code implements the key requirements from the blueprint."""
        errors = []
        
        # Get blueprint content (handle both natural and structured formats)
        blueprint_text = ""
        if hasattr(blueprint, 'raw_content'):
            blueprint_text = blueprint.raw_content.lower()
        elif hasattr(blueprint, 'description'):
            blueprint_text = blueprint.description.lower()
        
        code_lower = code.lower()
        
        # Check for FastAPI application patterns
        if 'fastapi' in blueprint_text:
            if 'fastapi(' not in code_lower and 'app = ' not in code_lower:
                errors.append("FastAPI application instance not found - blueprint mentions FastAPI but no app instance created")
        
        # Check for health check endpoint
        if 'health' in blueprint_text and 'endpoint' in blueprint_text:
            if not any(pattern in code for pattern in ['/health', '@app.get', 'async def health', 'def health']):
                errors.append("Health check endpoint missing - blueprint specifies health check but no endpoint found")
        
        # Check for middleware setup
        if 'middleware' in blueprint_text:
            if 'add_middleware' not in code_lower and 'middleware' not in code_lower:
                errors.append("Middleware configuration missing - blueprint mentions middleware but none configured")
        
        # Check for CORS setup
        if 'cors' in blueprint_text:
            if 'cors' not in code_lower:
                errors.append("CORS configuration missing - blueprint mentions CORS but not implemented")
        
        # Check for router inclusion (if blueprint references API modules)
        if hasattr(blueprint, 'blueprint_refs') and blueprint.blueprint_refs:
            api_refs = [ref for ref in blueprint.blueprint_refs if 'api' in str(ref)]
            if api_refs and 'include_router' not in code_lower:
                errors.append("API routers not included - blueprint references API modules but no routers included")
        
        # Check for lifecycle management
        if 'lifespan' in blueprint_text or ('startup' in blueprint_text and 'shutdown' in blueprint_text):
            if 'lifespan' not in code_lower and '@app.on_event' not in code_lower:
                errors.append("Application lifecycle management missing - blueprint mentions lifespan/startup but not implemented")
        
        # Check for error handling
        if 'error' in blueprint_text and 'handler' in blueprint_text:
            if 'exception_handler' not in code_lower:
                errors.append("Error handling missing - blueprint mentions error handling but no exception handlers found")
        
        if errors:
            return VerificationResult(
                success=False,
                error_type="blueprint",
                error_message="Blueprint requirements not implemented",
                suggestions=errors
            )
...
class GenerationVerifier:
    ...
    def verify_and_improve(
        ...
        code = generator.generate_single_blueprint(
        ...
        for attempt in range(self.max_retries):
            results = self.verifier.verify_all(
                code, tmp_file, self.project_root, main_md_path, blueprint
            )
            ...
            retry_prompt = self._create_retry_prompt(
                blueprint, context_parts, code, results, attempt, language
            )
            code = generator.generate_single_blueprint(
                blueprint, [retry_prompt], language, dependency_versions
            )
